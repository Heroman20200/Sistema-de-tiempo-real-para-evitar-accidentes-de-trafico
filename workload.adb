...
package body Workload is
  ...
  procedure Small_Whetstone (Kilo_Whets : in Positive) is
    ...
    T  : constant := 0.499_975; -- Values from the original Algol
    T1 : constant := 0.500_25;  --   Whetstone program and the
    ...
    Value     : constant := 0.941_377; -- Value calculated in main loop
    Tolerance : constant := 0.000_01;  -- Determined by interval arithmetic
    ...
    procedure P3 (X : in Whet_Float; Y : in Whet_Float; Z : in out Whet_Float)
    is
  ...
  begin -- Small_Whetstone
    ...
    if abs (Sum / Whet_Float (Kilo_Whets) - Value) >
     Tolerance * Whet_Float (Kilo_Whets)
    then
